<?php declare(strict_types=1);

/**
 * Example API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 * PHP version 8.1
 *
 * The version of the OpenAPI document: 1.0.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI-Generator
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 * Source files are located at:
 *
 * > https://github.com/OpenAPITools/openapi-generator/blob/master/modules/openapi-generator/src/main/resources/php-laravel/
 */


/**
 * Organization
 */
namespace Generated\DTO;

/**
 * Organization
 * @description Информация об организации, находящейся в здании.
 */

use Crell\Serde\Attributes as Serde;
use Crell\Serde\Renaming\Cases;

#[Serde\ClassSettings(renameWith: Cases::snake_case)]
class Organization
{
    /**
    *
    * Уникальный идентификатор организации.
    * @param int $id
    *
    * Название организации.
    * @param string $name
    *
    * ID здания, в котором находится организация.
    * @param int $buildingId
    *
    * Описание организации.
    * @param null | string $description
    *
    * Контактный email организации.
    * @param null | string $email
    *
    * Список контактных телефонов организации.
    * @param null | string[] $phones
    *
    * Виды деятельности, к которым относится организация.
    * @param null | \Generated\DTO\Activity[] $activities
    *
    * Дата создания записи.
    * @param null | \DateTime $createdAt
    *
    * Дата обновления записи.
    * @param null | \DateTime $updatedAt
    */

   public function __construct(
       public int $id,
       public string $name,
       public int $buildingId,
       public ?string $description = null,
       public ?string $email = null,
       public ?array $phones = null,
       #[Serde\SequenceField(arrayType: \Generated\DTO\Activity::class)]
       public ?array $activities = null,
       public ?\DateTime $createdAt = null,
       public ?\DateTime $updatedAt = null,
   ) {}
}

